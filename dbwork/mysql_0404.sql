...
-- mysql파일의 함수부분 복사해서 실행시켜 결과를 보세요
...
create table stu (
   ...
   idx smallint,
   sugangday date,
   constraint stu_fk_idx foreign key(idx) references bitclass(idx)); -- stu의 idx와 bitclass의 idx 조인
...
create table shop(
    ...
    sangpum varchar(30),
    price int,
    color varchar(20) default 'black'-- color값을 따로 안주면 자동으로 black저장
);
create table cart(
    ...
    -- on delete cascade를 했기 때문에 서브 테이블의 컬럼을 지우는데 제한이 없음
);
...
insert into shop values(null, '체크남방', 32999, 'pink');
select * from shop;
...
delete from shop where shopnum = 2;
