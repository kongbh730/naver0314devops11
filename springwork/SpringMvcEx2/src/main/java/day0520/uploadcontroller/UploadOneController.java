...
public class UploadOneController {
	...
	{
		//톰캣 서버에 배포된 프로젝트에서 이미지가 없로드 될 경로 구하기
		String realFolder=request.getSession().getServletContext().getRealPath("/resources/upload");
		System.out.println(realFolder);//콘솔로 출력 후 탐색기를 열어서 이미지 확인

		//업로드할 파일명
		//String fileName=upload.getOriginalFilename();//이미지명이 기존과 동일
		
		/*
		동시에 여러명이 서버에 접근해서 업로드할 경우 같은 파일명이 있을 경우
